...
| Dashboard recent activity (last 8 audit-log entries, admin-only — same gate as Audit Logs) | ✅ Done (2026-09-03) | `components/dashboard/RecentActivity.tsx` |
| Test infra (Jest + RTL) | ✅ 43 suites / 185 tests green | see [Gotcha #1](#1-jest-fails-with-module-jestsetupts-was-not-found) |
...
- **The Dashboard's "Recent activity" section was a permanent `EmptyState` stub**, never wired up. Fixed 2026-09-03 with `components/dashboard/RecentActivity.tsx` — reuses the Phase 9 audit-log infrastructure directly (`useAuditLogs({}, 8, 0, ...)`, no new backend endpoint needed) rather than building anything bespoke. **Design decision, not asked**: gated to admins only (`canViewAuditLogs`, same permission as the dedicated Audit Logs page) since it surfaces the same underlying data — the `useAuditLogs` request itself is disabled via a new optional `enabled` param for non-admins, not just the display, so it never fires an unnecessary/unauthorized request. Each entry reads as "{user} {verb} {article + entity}" (e.g. "Jane Doe created a project") with a relative timestamp; user names are resolved via the existing `useUsers()` hook, falling back to "Someone" if unresolvable. Caught and fixed a grammar bug before shipping: a naive "starts with a vowel letter → use 'an'" heuristic would say "an user" (wrong — English treats "user" as a consonant sound) and "a audit log" (wrong — needs "an"); fixed with an explicit `ENTITY_LABELS` lookup spelling out the correct article per entity type instead of deriving it.
...
- **`CreateProjectDialog` got cut off top and bottom with no way to scroll to Submit**, once the 2026-08-31 field additions (description/location/scale/crediting period) made the form taller than the viewport. Root cause: the shared `DialogContent` (`components/ui/dialog.tsx`) centers its popup with `top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2` and no height constraint at all — any dialog whose content exceeds the viewport gets clipped equally on both edges with no scroll container. Fixed 2026-09-03 by applying the same `max-h-[70vh] overflow-y-auto` pattern `CreateHouseholdDialog.tsx` already used to `CreateProjectDialog.tsx`'s form. Other multi-field dialogs (`CreateOrganisationDialog`, `CalculationFormDialog`) either already had this or are short enough not to need it — not touched.
...
### 6. The app is now deployed to a public dev environment — Vercel, CORS-gated

Live at **`https://verafi-web.vercel.app`** as of 2026-09-03. `.env.local` is gitignored (`.env*` in `.gitignore`, `.env.example` excepted) — it never reaches the deploy. Vercel's own dashboard env vars (`NEXT_PUBLIC_API_URL`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL` set to the Vercel URL itself) are what actually apply in production.

**The backend's CORS allowlist only included `http://localhost:3000` at first** — every browser-side API call from the deployed app 400'd with "Disallowed CORS origin" (confirmed directly: `curl -X OPTIONS .../methodologies -H "Origin: https://verafi-web.vercel.app"` returned exactly that, while the same call with `Origin: http://localhost:3000` returned 200). Login still worked throughout, since NextAuth's `authorize()` runs server-side in a Vercel function and isn't subject to browser CORS — only client-side `apiClient` calls were blocked, surfacing as a plain axios "Network Error" everywhere (`getErrorMessage`'s no-response branch). **Not a frontend bug or fix** — resolved by adding `https://verafi-web.vercel.app` to the backend's CORS origin allowlist. Worth remembering: an intermittent "login works but nothing else loads" symptom on any future deploy target is this exact class of issue — check CORS before assuming a frontend regression.
